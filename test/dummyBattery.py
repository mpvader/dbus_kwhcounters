...
def update():
	p = '/History/DischargedEnergy'
	dbusservice[p] = int(dbusservice[p]) + 1
	logging.info("value now for %s is %s" % (p, dbusservice[p]))

	p = '/History/ChargedEnergy'
	dbusservice[p] = int(dbusservice[p]) + 2
	logging.info("value now for %s is %s" % (p, dbusservice[p]))
...
dbusservice.add_path('/History/DischargedEnergy', 2)
dbusservice.add_path('/History/ChargedEnergy', 2000)

gobject.timeout_add(300, update)
